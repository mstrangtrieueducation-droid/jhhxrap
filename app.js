(() => {
  ...
  function renderReading() {
    const sections = new Map([...new Set(data.lines.map((line) => line.section))].map((name) => [name, document.createElement("section")]));
    sections.forEach((section) => {
      section.className = "reading-section";
    });
    data.lines.forEach((line) => sections.get(line.section).append(readingLine(line)));
  ...
  function setImage(name) {
    ...
    window.setTimeout(() => {
      image.src = `assets/images/${imageSources[name]}?v=20260806-discover2-hires-dtw-u1`;
  ...
  function updateKaraoke() {
    ...
    const lineData = data.lines.find((line) => time >= line.start && time <= line.end);
    $$(".reading-line").forEach((line) => {
      line.classList.toggle("is-current", Number(line.dataset.lineId) === lineData?.id);
    });
  ...
  function updatePlayState() {
    ...
    $("#playLabel").textContent = playing ? "Tạm dừng" : audio.ended ? "Nghe lại" : "Nghe bài đọc";
    $("#readingStatus").textContent = playing
      ? "Đang nghe track gốc và đọc theo"
      : audio.ended
        ? "Đã nghe xong · đến lượt con đọc lại"
  ...
  function readBack() {
    ...
    $("#mediaCaption").textContent =
      "Đang quay màn hình: con tự đọc rõ, đúng phát âm, trọng âm, nhịp ngắt và ngữ điệu.";
    $("#readingStatus").textContent = "Con tự đọc lại toàn bài để nộp video";
    $("#readingCopy").classList.add("is-readback");
  }
  ...
  $("#playButton").addEventListener("click", () => {
    $("#readingCopy").classList.remove("is-readback");
    $("#mediaCaption").textContent =
      "Con nghe kỹ phát âm, trọng âm, nhịp ngắt và ngữ điệu của track sách.";
    if (audio.paused) playFromStartIfEnded();
    else audio.pause();
  });
  $("#restartButton").addEventListener("click", resetAndPlay);
  $("#readBackButton").addEventListener("click", readBack);
